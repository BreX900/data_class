...
class _FieldSet implements Comparable<_FieldSet> {
  ...
  factory _FieldSet(FieldElement? classField, FieldElement? superField) {
    ...
    final fieldHasJsonKey = fields.firstWhere(hasDataFieldAnnotation, orElse: () => fields.first);
...
List<FieldElement> createSortedFieldSet(ClassElement element) {
  ...
  for (final v in manager.getInheritedConcreteMap2(element as InterfaceElementImpl).values) {
    assert(v is! FieldElement);
    if (_dartCoreObjectChecker.isExactly(v.enclosingElement3)) {
      continue;
    }

    if (v is PropertyAccessorElement && (v as PropertyAccessorElement).isGetter) {
